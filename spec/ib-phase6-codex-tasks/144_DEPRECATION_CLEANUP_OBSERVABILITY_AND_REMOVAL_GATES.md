# Task 144 — Deprecation cleanup, observability, and removal gates
...
Finalize consolidation by adding observability around deprecated paths and defining explicit removal gates before legacy IB code is deleted.
...
- Add instrumentation or logging around deprecated IB routes, endpoints, and adapters so the team can see whether they are still hit in pilot usage.
- Define removal gates such as zero traffic for a sustained period, successful backfill completion, redirect success, and support sign-off.
- Clean up obvious dead code that is now provably unused, but only after verifying no active references remain.
- Update documentation and runbooks to remove mention of deprecated paths once replacement is stable.
- Leave clear TODO/expiry markers where temporary compatibility layers remain.
...
- Deprecation observability and removal-gate definitions.
- Any safe cleanup of demonstrably dead code.
- Updated docs reflecting the consolidated architecture.

## Acceptance criteria

- There is a measurable way to know when a legacy path can be removed.
- Deprecated path traffic/usage is observable.
- Dead code removal is backed by evidence, not guesswork.
- Docs and runbooks no longer point operators to retired paths.
...
- Do not rush deletion just because a new path exists.
- Keep compatibility layers explicit and temporary.
- Use evidence-based removal gates.

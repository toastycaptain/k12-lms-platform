# Task 116 — Playwright e2e infrastructure, auth, and fixtures
...
Strengthen the e2e foundation so IB workflow suites run deterministically with stable auth, seeded data, and environment setup.
...
- Audit the current Playwright layout and create a clear convention for helpers, page objects (if used), fixtures, auth setup, and seeded IB pilot data.
- Provide deterministic test users for teacher, specialist, coordinator, admin, student, and guardian roles, scoped to a pilot school with the active IB pack.
- Stabilise login/session setup so e2e tests do not depend on brittle manual UI steps unless login flow itself is the subject under test.
- Seed or factory-build enough IB records to support PYP/MYP/DP paths, publishing queues, standards packets, readiness blockers, and pilot wizard states.
- Capture traces, screenshots, and console/network logs on failure in a way that makes CI triage practical.
- Document how local and CI environments should prepare the app and data for e2e runs.
...
- A deterministic e2e test infrastructure for the IB pilot baseline.
- Reusable role/scenario fixtures.
- Failure artifacts and documentation.

## Acceptance criteria

- The Playwright suite can run from a clean environment with predictable fixtures.
- Role-based auth setup is stable and reusable.
- Failures produce enough artifacts to debug quickly.
- The infrastructure supports the programme workflow suites that follow.
...
- Do not over-abstract page objects if direct test readability would suffer.
- Keep fixtures close to the real active IB pack and route tree.
- Minimise dependence on test ordering.

# Task 121 — Job inventory, queue topology, and idempotency rules
...
Map every IB-relevant background operation, define queue topology and retry semantics, and harden each path with explicit idempotency expectations.
...
- Inventory all jobs touching publishing, digests, readiness recomputation, exports, imports, telemetry, and operational backfills.
- Classify jobs by latency sensitivity, retry safety, concurrency risk, and actor visibility; use this to define or refine queue names and priorities.
- For each job type, document the idempotency key or natural uniqueness boundary (for example publishing queue item ID plus target state).
- Add guards to prevent duplicate processing when the same event/job is enqueued multiple times or retried after partial completion.
- Review side effects such as notifications, audit logs, and exports to ensure they are not duplicated on retry.
- Add focused job specs around idempotency and duplicate enqueue scenarios for the highest-risk operations.
...
- A job inventory/topology document or code-level configuration update.
- Idempotency guards and specs for critical IB jobs.
- Clear queue naming/prioritisation guidance.

## Acceptance criteria

- Critical IB jobs have explicit retry and idempotency stories.
- Queue topology reflects operational priority rather than default sprawl.
- Duplicate publish/export/notification side effects are prevented or detectable.
- The team can answer which queue/job handles each pilot-critical background operation.
...
- Do not rely on Sidekiq retries alone as the 'strategy'.
- Avoid a queue topology so granular it becomes unmanageable.
- Keep job contracts auditable and comprehensible.

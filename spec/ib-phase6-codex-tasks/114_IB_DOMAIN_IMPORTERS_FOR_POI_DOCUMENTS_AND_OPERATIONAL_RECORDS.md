# Task 114 — IB domain importers for POI, documents, and operational records
...
Build the actual domain-specific import executors for the highest-value IB entities, using the staged import pipeline and respecting pack/version/workflow rules.
...
- Implement initial importers for PYP Programme of Inquiry structures, curriculum documents (unit/course-map style records), planning-context associations, and operational records/checkpoints.
- Where possible, import into draft/review states rather than published states to preserve human review after migration.
- Respect active pack versioning when creating imported `CurriculumDocument` and `CurriculumDocumentVersion` records; do not import into stale schema contexts.
- Handle collaborator, advisor, owner, and route-hint fields where source data provides them, but validate user/entity existence carefully.
- Support partial imports by domain so a school can import POI first, then documents, then operational records later.
- Add service specs using realistic staged data for each supported import domain.
...
- Domain-specific import executors for the first wave of IB entities.
- Service specs proving correct creation/linking/pinning behaviour.
- Documentation of supported import shapes and intentional omissions.

## Acceptance criteria

- The initial supported import domains can create meaningful IB pilot data from staged inputs.
- Imported records land in safe workflow states and preserve audit context.
- Pack pinning, planning-context links, and owner assignments are correct for imported records.
- Unsupported fields are surfaced clearly rather than silently discarded.
...
- Do not attempt a perfect universal importer for every historical system in this task.
- Prioritise safe and useful imports over broad but brittle coverage.
- Keep importers modular so later MYP/DP-specific enrichments can be added cleanly.

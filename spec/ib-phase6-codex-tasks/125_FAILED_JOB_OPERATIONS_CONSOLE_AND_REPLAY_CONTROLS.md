# Task 125 — Failed-job operations console and replay controls
...
Expose background failure state and safe replay/recovery actions through admin/coordinator tooling so pilot operations do not depend on shell access.
...
- Identify which failed operations should be visible to which role: admins, coordinators, or support staff.
- Add backend endpoints or services that surface failed jobs/operations in domain language rather than raw queue internals where possible.
- Provide operator actions such as retry, requeue, mark resolved after manual fix, or open related record context—guarded by permission and audit logging.
- Show last failure time, reason, retry count, related entity, and recommended next action in the UI.
- Integrate failed-job visibility into rollout/readiness/support dashboards so operators can see system health holistically.
- Write request and frontend tests around permissioning and replay safety.
...
- An operations console or support panel for failed jobs/queues.
- Safe replay/recovery controls with audit coverage.
- Tests for visibility and mutation actions.

## Acceptance criteria

- Operators can diagnose and act on failed background operations without engineering shell access.
- Replay actions are permissioned, audited, and safe to retry.
- The console uses domain terms (publish/export/import/etc.) rather than only generic queue IDs.
- Failure state is integrated into readiness/support views.
...
- Do not expose dangerous low-level queue mutation controls casually.
- Avoid making the console a raw Sidekiq clone; tie it back to IB domain entities.
- Every replay action must explain what it will do.

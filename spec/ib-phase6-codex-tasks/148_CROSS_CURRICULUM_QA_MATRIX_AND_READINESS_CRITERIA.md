# Task 148 — Cross-curriculum QA matrix and readiness criteria
...
Define how the newly extracted shared modules will be validated for reuse in future curricula without confusing this phase with a full American/British buildout.
...
- Build a QA matrix that distinguishes IB-specific tests from shared-module tests so reuse work can proceed safely later.
- Define readiness criteria for calling a module 'cross-curriculum ready': pack-neutral contract coverage, no hardcoded IB labels, role/permission agnosticism where appropriate, and docs/examples.
- Identify which existing tests can be lifted into shared suites and which must remain IB-only.
- Document how future American/British work should consume the extracted modules without bypassing the readiness criteria.
- Add any immediate missing shared tests that protect the extracted seams.
...
- A cross-curriculum QA matrix and readiness rubric.
- Any immediate shared tests needed for extracted modules.
- Documentation for future curriculum-phase teams.

## Acceptance criteria

- The team has a concrete standard for when a module is safe to reuse beyond IB.
- Shared and IB-only tests are clearly distinguished.
- Future work can build on explicit readiness criteria rather than assumptions.
- No one confuses 'module extracted' with 'other curricula fully supported'.
...
- Do not spend this task implementing American/British features.
- Keep the matrix practical and aligned to actual extracted modules.
- Use the rubric to prevent premature reuse claims.

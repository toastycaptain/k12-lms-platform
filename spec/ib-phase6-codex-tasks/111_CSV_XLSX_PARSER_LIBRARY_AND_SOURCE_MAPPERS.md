# Task 111 — CSV/XLSX parser library and source mappers
...
Build the file ingestion layer for spreadsheet-based imports, including parser abstraction, source-type adapters, and normalised row representations.
...
- Support at minimum CSV and XLSX ingestion for common pilot migration shapes, with a parser abstraction that returns normalised row objects plus source metadata.
- Handle column headings, duplicate columns, blank rows, merged-cell weirdness, unicode text, and spreadsheet date coercion explicitly; do not trust parser defaults blindly.
- Create source mappers for likely import domains such as POI entries, unit metadata, course maps, staff-role assignments, evidence/story metadata, and operational checkpoints.
- Capture parser warnings distinctly from validation failures so admins can see 'messy file' versus 'invalid business mapping'.
- Attach row-level provenance (sheet name, row number, column name) that can later power conflict reporting and UI highlighting.
- Add service tests with representative messy fixtures, not just ideal spreadsheets.
...
- Parser/services capable of reading CSV and XLSX into staged import rows.
- Source-specific mapping adapters for the initial IB import targets.
- Tests and example fixtures covering real spreadsheet quirks.

## Acceptance criteria

- Staged rows preserve enough provenance to explain parse issues precisely.
- The parser layer is isolated from final business writes.
- Common spreadsheet edge cases are handled or surfaced predictably.
- The same import architecture can be fed by multiple spreadsheet shapes.
...
- Do not assume all pilots will deliver clean canonical spreadsheets.
- Avoid hardcoding one vendor export format as the only accepted shape.
- Keep raw parsing separate from business validation.

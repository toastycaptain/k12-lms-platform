class QuizPolicy < ApplicationPolicy
  ...
  def show?
    privileged_user? || owns_quiz? || teaches_course?(record.course_id) || student_can_view?
  end
  ...
  def update?
    privileged_user? || owns_quiz? || teaches_course?(record.course_id)
  end
  ...
  class Scope < ApplicationPolicy::Scope
    def resolve
      ...
      if user.has_role?(:student)
        return scope.where(course_id: enrolled_student_course_ids, status: "published")
      end

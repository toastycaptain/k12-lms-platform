class AiRewriteJob < ApplicationJob
  ...
  def build_rewrite_prompt(params)
    parts = [ "Rewrite the following content according to these instructions: #{params['instruction']}" ]
    parts << "Content to rewrite:"

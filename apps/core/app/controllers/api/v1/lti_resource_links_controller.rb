module Api
  module V1
    class LtiResourceLinksController < ApplicationController
      ...
      def index
        links = policy_scope(LtiResourceLink).where(lti_registration: @lti_registration)
        render json: links
      end
      ...
      def create
        @lti_resource_link = @lti_registration.lti_resource_links.build(lti_resource_link_params)
      ...
      def lti_resource_link_params
        params.permit(:title, :description, :url, :course_id, custom_params: {})
      end
    end
  end
end

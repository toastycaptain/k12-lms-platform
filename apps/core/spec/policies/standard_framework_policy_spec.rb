...
RSpec.describe StandardFrameworkPolicy, type: :policy do
  ...
  let(:record) { create(:standard_framework, tenant: tenant) }
  ...
  describe "Scope" do
    let!(:f1) { create(:standard_framework, tenant: tenant) }
    let!(:f2) { create(:standard_framework, tenant: tenant) }

    it "returns all records" do
      expect(described_class::Scope.new(teacher, StandardFramework).resolve).to contain_exactly(f1, f2)
    end
  end
end

...
RSpec.describe SectionPolicy, type: :policy do
  ...
  let(:record) { create(:section, tenant: tenant) }
  ...
  describe "Scope" do
    let!(:section1) { create(:section, tenant: tenant) }
    let!(:section2) { create(:section, tenant: tenant) }

    it "returns all records" do
      expect(described_class::Scope.new(teacher, Section).resolve).to contain_exactly(section1, section2)
    end
  end
end

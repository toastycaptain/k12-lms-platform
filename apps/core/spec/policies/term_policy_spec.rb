...
RSpec.describe TermPolicy, type: :policy do
  ...
  let(:record) { create(:term, tenant: tenant) }
  ...
  describe "Scope" do
    let!(:t1) { create(:term, tenant: tenant) }
    let!(:t2) { create(:term, tenant: tenant) }

    it "returns all records" do
      expect(described_class::Scope.new(teacher, Term).resolve).to contain_exactly(t1, t2)
    end
  end
end

...
RSpec.describe RubricPolicy, type: :policy do
  ...
  let(:record) { create(:rubric, tenant: tenant, created_by: teacher) }
  ...
  describe "Scope" do
    let!(:rubric1) { create(:rubric, tenant: tenant, created_by: teacher) }
    let!(:rubric2) { create(:rubric, tenant: tenant, created_by: teacher) }

    it "returns all records" do
      expect(described_class::Scope.new(student, Rubric).resolve).to contain_exactly(rubric1, rubric2)
    end
  end
end
